...
│   │   test_request.py								# script to test the model API
│   │
│   ├───services
│   │   │   recommender_engine.py					# processing & modeling logic
|
...
### **4. API of Hybrid Recommendation Model**

- Set Flask app & Run
```bash
set FLASK_APP=MODEL_API_dynamic_questions.app
...
> Note: Change `BASE_DATA_PATH` to any file in the `base_questions` folder and change `DYNAMIC_DATA_PATH` to any file in the `ANSWER_RESPONSE_dynamic_questions` to test different initial inputs.
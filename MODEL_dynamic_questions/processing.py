...
def validate_response_completeness(data: Dict[str, Any], expected_count: int = 6) -> None:
    ...
    if len(responses) != expected_count:
        raise ValueError(f"Validation Error: Expected exactly {expected_count} competency responses, but found {len(responses)}.")
...
def build_user_persona_summary(user_profile_json: Dict[str, Any]) -> str:
    ...
    situation = responses.get("Q1_current_status", {}).get("answer", "").strip()
    interest_area = responses.get("Q4_interest_area", {}).get("answer", "").strip()
    skill_level = responses.get("Q6_skill_level", {}).get("answer", "").strip()
    time = responses.get("Q9_time_commitment", {}).get("answer", "").strip()
    persona_summary = (
...
def build_persona_flags(data: Dict[str, Any]) -> Dict[str, bool]:
    ...
    # Determine flags using substring checks
...
def mapping_competency_scores(data: Dict[str, Any]) -> Dict[str, Union[Dict[str, float], Tuple[float, ...]]]:
    full_data = data
    responses = full_data.get('user_profile', {}).get('responses', [])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Builds final payload from base + dynamic JSONs.")
    parser.add_argument('--base', type=str, required=True, help="Path to base user persona JSON")
    parser.add_argument('--dynamic', type=str, required=True, help="Path to dynamic answers JSON")
    parser.add_argument('--output', type=str, default="FINAL_PAYLOAD/SAMPLEE_final_payload.json", help="Output final payload JSON")
